...
<script>
    /**Fonctions */
    const rightButton = (data, element, offset) => {
        let newOffset;
    ...
    eyesRight.addEventListener('click', function() {
        eyesOffset = rightButton(eyes, 'eyes', eyesOffset);
    });
    mouthsRight.addEventListener('click', function() {
        mouthsOffset = rightButton(mouths, 'mouths', mouthsOffset);
    });
    posesRight.addEventListener('click', function() {
        posesOffset = rightButton(poses, 'poses', posesOffset);
...
@section('contenu')
<x-avatar-editor label="Valider et continuer" route="{{route('createAvatar')}}"></x-avatar-editor>
@endsection